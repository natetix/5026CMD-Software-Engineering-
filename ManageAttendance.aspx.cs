...
namespace StudentManagementSystem
{
    public partial class ManageAttendance : System.Web.UI.Page
    {
        ...
        EventArgs e)
        {
            // Guard: course must be selected and date must be valid before inserting
            if (string.IsNullOrEmpty(ddlCourse.SelectedValue))
            {
                lblMessage.Text = "Please select a session and course first.";
                return;
            }

            DateTime attDate;
            if (!DateTime.TryParse(txtDate.Text, out attDate))
            {
                lblMessage.Text = "Please enter a valid attendance date.";
                return;
            }
